...
<body>
  ...
  <script>
    ...
    const markdownLatexSuiteEnabled = supportsMarkdownPreview;
    ...
    const setupMarkdownLatexSuite = () => {
      if (!markdownLatexSuiteEnabled) return;

      const Range = ace.require('ace/range').Range;
      const latexSuiteTextSnippets = [
        {
          trigger: 'mk',
          template: '$[[0]]$[[1]]',
          boundary: true,
        ...
        {
          trigger: 'dm',
          template: (lineBefore) => (lineBefore.trim() ? '\\n$$\\n\\t[[0]]\\n$$[[1]]' : '$$\\n\\t[[0]]\\n$$[[1]]'),
          boundary: true,
        },
      ];
      const latexSuiteMathSnippets = [
        { trigger: 'invs', template: '^{-1}' },
        { trigger: 'text', template: '\\\\text{[[0]]}[[1]]' },
        { trigger: 'ome', template: '\\\\omega' },
        { trigger: 'Ome', template: '\\\\Omega' },
        { trigger: 'sr', template: '^{2}' },
        { trigger: 'cb', template: '^{3}' },
        { trigger: 'rd', template: '^{[[0]]}[[1]]' },
        { trigger: 'sq', template: '\\\\sqrt{ [[0]] }[[1]]' },
        { trigger: '//', template: '\\\\frac{[[0]]}{[[1]]}[[2]]' },
        { trigger: 'ee', template: 'e^{ [[0]] }[[1]]' },
        { trigger: '@a', template: '\\\\alpha' },
        { trigger: '@b', template: '\\\\beta' },
        { trigger: '@g', template: '\\\\gamma' },
        { trigger: '@G', template: '\\\\Gamma' },
        { trigger: '@d', template: '\\\\delta' },
        { trigger: '@D', template: '\\\\Delta' },
        { trigger: '@e', template: '\\\\epsilon' },
        { trigger: ':e', template: '\\\\varepsilon' },
        { trigger: '@z', template: '\\\\zeta' },
        { trigger: '@t', template: '\\\\theta' },
        { trigger: '@T', template: '\\\\Theta' },
        { trigger: ':t', template: '\\\\vartheta' },
        { trigger: '@i', template: '\\\\iota' },
        { trigger: '@k', template: '\\\\kappa' },
        { trigger: '@l', template: '\\\\lambda' },
        { trigger: '@L', template: '\\\\Lambda' },
        { trigger: '@s', template: '\\\\sigma' },
        { trigger: '@S', template: '\\\\Sigma' },
        { trigger: '@u', template: '\\\\upsilon' },
        { trigger: '@U', template: '\\\\Upsilon' },
        { trigger: '@o', template: '\\\\omega' },
        { trigger: '@O', template: '\\\\Omega' },
        { trigger: '"', template: '\\\\text{[[0]]}[[1]]' },
        { trigger: '_', template: '_{[[0]]}[[1]]', skipEscaped: true },
      ].sort((left, right) => right.trigger.length - left.trigger.length);
      let latexSuiteApplyingChange = false;
      let latexSuiteTabstops = [];
      let latexSuiteTabstopIndex = 0;

      const positionToIndex = (position) => editor.session.doc.positionToIndex(position, 0);
      const indexToPosition = (index) => editor.session.doc.indexToPosition(index, 0);
      const isBackslashEscaped = (text, index) => {
        let count = 0;
      ...
      const disposeLatexSuiteTabstops = () => {
        latexSuiteTabstops.forEach((stop) => {
          try {
            stop.startAnchor.detach();
            stop.endAnchor.detach();
          } catch {
            // Ace anchors may already be detached after document resets.
          }
        });
        latexSuiteTabstops = [];
        latexSuiteTabstopIndex = 0;
      };
      const createLatexSuiteAnchor = (index, insertRight) => {
        const position = indexToPosition(index);
        const anchor = editor.session.doc.createAnchor(position.row, position.column);
        anchor.$insertRight = Boolean(insertRight);
        return anchor;
      };
      const anchorToPosition = (anchor) => ({ row: anchor.row, column: anchor.column });
      const parseLatexSuiteTemplate = (template) => {
        const stops = [];
        let text = '';
        let consumed = 0;
        const pattern = /\\[\\[(\\d+)(?::([^\\]]*))?\\]\\]/g;
        ...
        while (match) {
          text += template.slice(consumed, match.index);
        ...
        text += template.slice(consumed);
        stops.sort((left, right) => left.order - right.order || left.startOffset - right.startOffset);
        return { text, stops };
      };
      const selectLatexSuiteTabstop = (stop) => {
        const start = anchorToPosition(stop.startAnchor);
        const end = anchorToPosition(stop.endAnchor);
        editor.selection.setRange(new Range(start.row, start.column, end.row, end.column), false);
        editor.focus();
      };
      const insertLatexSuiteTemplate = (start, end, template) => {
        const parsed = parseLatexSuiteTemplate(template);
        const startIndex = positionToIndex(start);
        disposeLatexSuiteTabstops();
        latexSuiteApplyingChange = true;
        try {
          editor.session.replace(new Range(start.row, start.column, end.row, end.column), parsed.text);
        } finally {
          latexSuiteApplyingChange = false;
        }

        latexSuiteTabstops = parsed.stops.map((stop) => ({
          startAnchor: createLatexSuiteAnchor(startIndex + stop.startOffset, false),
          endAnchor: createLatexSuiteAnchor(startIndex + stop.endOffset, true),
        }));

        if (latexSuiteTabstops.length > 0) {
          latexSuiteTabstopIndex = 0;
          selectLatexSuiteTabstop(latexSuiteTabstops[0]);
          return;
        }

        const cursor = indexToPosition(startIndex + parsed.text.length);
        editor.moveCursorTo(cursor.row, cursor.column);
        editor.clearSelection();
      };
      const hasLatexSuiteBoundaryBefore = (line, startColumn) => {
        if (startColumn <= 0) return true;
        return /[\\s([{'"*_$]/.test(line.charAt(startColumn - 1));
      };
      const isInsideMarkdownFence = (index) => {
        const text = editor.session.getValue().slice(0, index);
        const lines = text.split('\\n');
        let fence = '';
        for (const line of lines) {
          const match = line.match(/^\\s*((?:\\x60){3,}|~~~+)/);
          if (!match) continue;
          const marker = match[1].charCodeAt(0) === 96 ? 'tick' : '~';
          if (!fence) {
            fence = marker;
      ...
      const findMarkdownMathBounds = (index) => {
        const text = editor.session.getValue();
        if (isInsideMarkdownFence(index)) return null;

        let inlineStart = -1;
        let displayStart = -1;
        for (let cursor = 0; cursor < text.length; cursor += 1) {
          if (text.charAt(cursor) !== '$' || isBackslashEscaped(text, cursor)) continue;

          if (text.slice(cursor, cursor + 2) === '$$') {
            if (displayStart >= 0) {
              if (index >= displayStart + 2 && index <= cursor) {
                return {
                  innerStart: displayStart + 2,
                  innerEnd: cursor,
                  outerStart: displayStart,
                  outerEnd: cursor + 2,
                  mode: 'display',
                };
              }
          ...
          if (inlineStart >= 0) {
            if (index >= inlineStart + 1 && index <= cursor) {
              return {
        ...
        if (displayStart >= 0 && index >= displayStart + 2) {
          return {
            innerStart: displayStart + 2,
        ...
        if (inlineStart >= 0 && index >= inlineStart + 1) {
          return {
            innerStart: inlineStart + 1,
            innerEnd: text.length,
            outerStart: inlineStart,
      ...
      const currentMarkdownMathBounds = () => findMarkdownMathBounds(positionToIndex(editor.getCursorPosition()));
      const isMarkdownMathCursor = () => Boolean(currentMarkdownMathBounds());
      const isMarkdownTextCursor = () => {
        const index = positionToIndex(editor.getCursorPosition());
        return !isInsideMarkdownFence(index) && !findMarkdownMathBounds(index);
      };
      const tryExpandLatexSuiteSnippet = (snippet) => {
        if (!editor.selection.isEmpty()) return false;
        const cursor = editor.getCursorPosition();
        const line = editor.session.getLine(cursor.row);
        const trigger = snippet.trigger;
        const startColumn = cursor.column - trigger.length;
        if (startColumn < 0 || line.slice(startColumn, cursor.column) !== trigger) return false;
        if (snippet.skipEscaped && isBackslashEscaped(line, startColumn)) return false;
        if (snippet.boundary && !hasLatexSuiteBoundaryBefore(line, startColumn)) return false;

        const start = { row: cursor.row, column: startColumn };
        const lineBefore = line.slice(0, startColumn);
        const template = typeof snippet.template === 'function' ? snippet.template(lineBefore) : snippet.template;
        insertLatexSuiteTemplate(start, cursor, template);
        return true;
      };
      const hasBalancedOuterParentheses = (value) => {
      ...
      const normalizeLatexSuiteNumerator = (value) => {
        const trimmed = value.trim();
        return hasBalancedOuterParentheses(trimmed) ? trimmed.slice(1, -1).trim() : trimmed;
      };
      const findLatexSuiteNumeratorStart = (lineBeforeSlash) => {
        let depth = 0;
        for (let index = lineBeforeSlash.length - 1; index >= 0; index -= 1) {
          const char = lineBeforeSlash.charAt(index);
          if (char === ')' || char === ']' || char === '}') {
            depth += 1;
            continue;
          }
      ...
      const tryExpandAutoFraction = () => {
        if (!editor.selection.isEmpty() || !isMarkdownMathCursor()) return false;
        const cursor = editor.getCursorPosition();
        if (cursor.column <= 0) return false;
        const line = editor.session.getLine(cursor.row);
        if (line.charAt(cursor.column - 1) !== '/') return false;
        const lineBeforeSlash = line.slice(0, cursor.column - 1);
        const startColumn = findLatexSuiteNumeratorStart(lineBeforeSlash);
        const numerator = normalizeLatexSuiteNumerator(lineBeforeSlash.slice(startColumn));
        if (!numerator || numerator === '/') return false;
        insertLatexSuiteTemplate(
          { row: cursor.row, column: startColumn },
          cursor,
          '\\\\frac{' + numerator + '}{[[0]]}[[1]]',
        );
        return true;
      };
      const tryConvertSlashFractionBeforeCursor = () => {
        if (!editor.selection.isEmpty() || !isMarkdownMathCursor()) return false;
        const cursor = editor.getCursorPosition();
        const line = editor.session.getLine(cursor.row);
        const prefix = line.slice(0, cursor.column);
        const slashColumn = prefix.lastIndexOf('/');
        if (slashColumn <= 0 || slashColumn === prefix.length - 1 || isBackslashEscaped(prefix, slashColumn)) return false;
        const startColumn = findLatexSuiteNumeratorStart(prefix.slice(0, slashColumn));
        const numerator = normalizeLatexSuiteNumerator(prefix.slice(startColumn, slashColumn));
        const denominator = prefix.slice(slashColumn + 1).trim();
        if (!numerator || !denominator || /\\s/.test(denominator)) return false;
        insertLatexSuiteTemplate(
          { row: cursor.row, column: startColumn },
          cursor,
          '\\\\frac{' + numerator + '}{' + denominator + '}[[0]]',
        );
        return true;
      };
      const moveLatexSuiteTabstop = (direction) => {
        if (latexSuiteTabstops.length === 0) return false;
        const nextIndex = latexSuiteTabstopIndex + direction;
        if (nextIndex < 0) return false;
        if (nextIndex >= latexSuiteTabstops.length) {
          disposeLatexSuiteTabstops();
          return false;
        }
        latexSuiteTabstopIndex = nextIndex;
        selectLatexSuiteTabstop(latexSuiteTabstops[latexSuiteTabstopIndex]);
        return true;
      };
      const tryLatexSuiteTabout = () => {
        const bounds = currentMarkdownMathBounds();
        if (!bounds) return false;
        const cursorIndex = positionToIndex(editor.getCursorPosition());
        const text = editor.session.getValue();
        const closingCommands = ['\\\\rangle', '\\\\rvert', '\\\\rVert'];
        for (let index = cursorIndex; index < bounds.innerEnd; index += 1) {
          if (isBackslashEscaped(text, index)) continue;
          const char = text.charAt(index);
          if (char === '}' || char === ')' || char === ']') {
            const next = indexToPosition(index + 1);
            editor.moveCursorTo(next.row, next.column);
            editor.clearSelection();
            return true;
          }
          for (const command of closingCommands) {
            if (text.slice(index, index + command.length) === command) {
              const next = indexToPosition(index + command.length);
              editor.moveCursorTo(next.row, next.column);
              editor.clearSelection();
              return true;
            }
        ...
        if (text.slice(cursorIndex, bounds.innerEnd).trim().length > 0) return false;
        const next = indexToPosition(bounds.outerEnd);
        editor.moveCursorTo(next.row, next.column);
        editor.clearSelection();
        return true;
      };
      const processLatexSuiteInput = (insertedText) => {
        if (latexSuiteApplyingChange || !editor.selection.isEmpty()) return;
        if (isMarkdownTextCursor()) {
          for (const snippet of latexSuiteTextSnippets) {
            if (tryExpandLatexSuiteSnippet(snippet)) return;
          }
          return;
        }
        if (!isMarkdownMathCursor()) return;
        for (const snippet of latexSuiteMathSnippets) {
          if (tryExpandLatexSuiteSnippet(snippet)) return;
        }
        if (insertedText === '/') {
          tryExpandAutoFraction();
        }
      };

      editor.session.on('change', (delta) => {
        if (latexSuiteApplyingChange || !delta || delta.action !== 'insert') return;
        if (!delta.lines || delta.lines.length !== 1 || delta.lines[0].length !== 1) return;
        processLatexSuiteInput(delta.lines[0]);
      });
      editor.container.addEventListener('keydown', (event) => {
        ...
        if (event.shiftKey) {
          if (!moveLatexSuiteTabstop(-1)) return;
        } else if (!moveLatexSuiteTabstop(1) && !tryConvertSlashFractionBeforeCursor() && !tryLatexSuiteTabout()) {
          return;
        }
        event.preventDefault();
        event.stopPropagation();
